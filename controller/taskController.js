...
async function syncSubtasks(c, taskId, items) {
  ...
  for (const item of items) {
    if (!item || typeof item !== 'object' || !validTitle(item.title) || !validCompleted(item.is_completed)) throw Object.assign(new Error('Invalid subtask'),{status:400});
    ...
    if (sid) { seen.push(sid); await c.query('UPDATE subtasks SET title=?,is_completed=? WHERE id=? AND task_id=?',[item.title.trim(),item.is_completed===true||item.is_completed===1?1:0,sid,taskId]); }
    else await c.query('INSERT INTO subtasks (task_id,title,is_completed) VALUES (?,?,?)',[taskId,item.title.trim(),item.is_completed===true||item.is_completed===1?1:0]);
  }
